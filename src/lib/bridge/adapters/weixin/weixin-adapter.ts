...
import { downloadMediaFromItem } from './weixin-media.js';
import { clearAllPauses, isPaused, setPaused } from './weixin-session-guard.js';
import type {
  GetUpdatesResponse,
  WeixinCredentials,
  WeixinMessage,
...
export class WeixinAdapter extends BaseChannelAdapter {
  ...
  private recentMessagesCache = new Map<string, number>();
  ...
  private async processMessage(accountId: string, message: WeixinMessage, batchId?: number): Promise<void> {
    ...
    const contentKey = `${chatId}:${inbound.text.trim()}`;
    const now = Date.now();
    const lastSeen = this.recentMessagesCache.get(contentKey);
    if (lastSeen && (now - lastSeen) < 15_000) {
      console.log(`[weixin-adapter] Duplicate message content detected within 15s for ${chatId}, ignoring`);
      return;
    }
    this.recentMessagesCache.set(contentKey, now);

    // Periodic cleanup of the recent messages cache to avoid memory leak
    if (this.recentMessagesCache.size > 500) {
      for (const [k, t] of this.recentMessagesCache.entries()) {
        if (now - t > 60_000) {
          this.recentMessagesCache.delete(k);
        }
      }
    }
